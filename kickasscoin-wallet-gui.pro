...
# cleaning "auto-generated" bitkickasscoin directory on "make distclean"
...
linux {
   ...
   #     }
    ...
    # currently kickasscoin has an issue with "static" build and linunwind-dev,
...
# translation stuff
TRANSLATIONS = $$files($$PWD/translations/kickasscoin-core_*.ts)
...
android{
    deploy.commands += make install INSTALL_ROOT=$$DESTDIR && androiddeployqt --input android-libkickasscoin-wallet-gui.so-deployment-settings.json --output $$DESTDIR --deployment bundled --android-platform android-21 --jdk /usr/lib/jvm/java-8-openjdk-amd64 -qmldir=$$PWD
}
...
DISTFILES += \
    notes.txt \
    kickasscoin/src/wallet/CMakeLists.txt
