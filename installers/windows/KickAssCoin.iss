...
; KickAssCoin Boron Butterfly GUI Wallet Installer for Windows
; Copyright (c) 2017-2019, The KickAssCoin Project
; See LICENSE

[Setup]
AppName=KickAssCoin GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.14.1.0
DefaultDirName={pf}\KickAssCoin GUI Wallet
DefaultGroupName=KickAssCoin GUI Wallet
UninstallDisplayIcon={app}\kickasscoin-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The KickAssCoin Developer Community
AppPublisherURL=https://getkickasscoin.org
...
; Background info:
; This installer indeed asks for admin rights so the KickAssCoin files can be copied to a place where they have at least
...
; As of version 0.14.1.0, the KickAssCoin .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to KickAssCoin.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; KickAssCoin GUI wallet exe and guide
Source: "bin\kickasscoin-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickasscoin-GUI-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; KickAssCoin CLI wallet
Source: "bin\kickasscoin-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickasscoin-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; KickAssCoin wallet RPC interface implementation
Source: "bin\kickasscoin-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; KickAssCoin daemon
Source: "bin\kickasscoind.exe"; DestDir: "{app}"; Flags: ignoreversion

; KickAssCoin daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "kickasscoin-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; KickAssCoin blockchain utilities
Source: "bin\kickasscoin-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickasscoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickasscoin-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickasscoin-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickasscoin-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickasscoin-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickasscoin-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickasscoin-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickasscoin-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\kickasscoin-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\kickasscoin-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitkickasscoin');
  s := 'The default folder to store the KickAssCoin blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\kickasscoin-project\kickasscoin-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" KickAssCoin default location
...
begin
  Result := BlockChainDir('') + '\bitkickasscoin.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "kickasscoin-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the KickAssCoin daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\kickasscoind.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\kickasscoin-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "KickAssCoin GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\kickasscoin-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\kickasscoin-GUI-guide.pdf"; IconFilename: "{app}\kickasscoin-wallet-gui.exe"
...
Name: "{group}\Utilities\KickAssCoin Daemon"; Filename: "{app}\kickasscoind.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\kickasscoin-wallet-cli.exe"; WorkingDir: "{userdocs}\KickAssCoin\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\KickAssCoin\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\kickasscoin-wallet-gui\kickasscoin-wallet-gui.log"""
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\kickasscoin-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM kickasscoind.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\kickasscoin-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\kickasscoin-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\kickasscoin-project\kickasscoin-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\kickasscoin-project\kickasscoin-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "kickasscoin:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "kickasscoin://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "kickasscoin"; ValueType: "string"; ValueData: "URL:KickAssCoin Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "kickasscoin"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "kickasscoin\DefaultIcon"; ValueType: "string"; ValueData: "{app}\kickasscoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "kickasscoin\shell\open\command"; ValueType: "string"; ValueData: """{app}\kickasscoin-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "kickasscoinseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "kickasscoinseed://a.b.c.d:port"
Root: HKCR; Subkey: "kickasscoinseed"; ValueType: "string"; ValueData: "URL:KickAssCoin Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "kickasscoinseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "kickasscoinseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\kickasscoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "kickasscoinseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\kickasscoin-wallet-gui.exe"" ""%1"""
