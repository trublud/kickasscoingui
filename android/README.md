...
# Base environnement
        ...
        cd kickasscoin/utils/build_scripts
        docker build -f android32.Dockerfile -t kickasscoin-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t kickasscoin-gui-android .
        docker create -it --name kickasscoin-gui-android kickasscoin-gui-android bash

# Get the apk

        docker cp kickasscoin-gui-android:/opt/android/kickasscoin-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/kickasscoin-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
