...
KICKASSCOIN_DIR=kickasscoin
KICKASSCOIND_EXEC=kickasscoind
...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/kickasscoin-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    KICKASSCOIND_EXEC=kickasscoind.exe
fi
...
get_tag
popd
echo "var GUI_KICKASSCOIN_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../kickasscoin-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy kickasscoind to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$KICKASSCOIN_DIR/bin/$KICKASSCOIND_EXEC $BIN_PATH
fi
